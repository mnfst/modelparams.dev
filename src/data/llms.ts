...
function guideApi(siteUrl: string): string[] {
  return [
    ...
    "## MCP server",
    "",
    "Agents can query the catalog over the Model Context Protocol instead of HTTP. The",
    "server runs on stdio and bundles the catalog, so it needs no network access:",
    "",
    "```bash",
